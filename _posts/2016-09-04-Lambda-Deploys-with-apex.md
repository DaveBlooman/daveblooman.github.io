...
The word serverless is used to describe your usage, rather than Amazons, as everything still runs on their servers.  Another way of describing Lambda is functions as a service, running only when you define in event actions or based on other service calls.  An example would be sending a Cloudwatch message to slack if a Cloudwatch alarm goes into alarm state for more than 5 minutes.
...
If you want to run any kind of complex function, you will have to upload dependencies, AWS lets you have up 250mb(compressed).  This will mean that using just the console is out, so using a single static binary works nicely in this situation.  
